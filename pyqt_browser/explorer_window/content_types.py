...
class File(object):
    ...
    def thumbnail(self,width=None,height=None,frame=None,indexF=None):
        ...
        icon_path = r"\\botcentral\BotScripts\Software\staging\Bot_Browser\show_browser\v02\project_browser\images"
        icon_file = re.sub('application/', '', mimetype[0])+".png"
        icon_filepath = os.path.abspath(os.path.join(icon_path, icon_file))
        print (os.path.exists(icon_filepath), icon_filepath)
        if not os.path.exists(icon_filepath):
            icon_filepath = os.path.join(icon_path, "project_browser.png")
        return imagemagick.make_thumbnail(icon_filepath)
