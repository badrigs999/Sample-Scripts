...
class Project_Model(QAbstractItemModel):
    ...
    def work_done(self,work):
        # print '***loaded', (work, projects.Project_Item)
    ...
    def update_item_view(self,item):
        ...
        # print ("item.row", item.columnCount(), item.row(), item, index_start)
...
class Project_Navigator_Widget(QWidget):
    ...
    def __init__(self,parent=None):
        ...
        h1_layout.addWidget(self.upButton)
        h1_layout.addWidget(self.backButton)
        h1_layout.addWidget(self.forwardButton)
        h1_layout.addWidget(self.path)
        #
        h1_layout.addSpacing(10)
        #h1_layout.addWidget(self.loadingIcon)
        # h1_layout.addWidget(self.load_preview)
        h1_layout.addWidget(self.refreshButton)
        h1_layout.addWidget(self.browseButton)
        #h1_layout.addWidget(self.menuButton)
        ...
        item_preview_value = settings.load_preview_mode("item_preview")
        seq_preview_value = False
        settings.save_preview_mode("seq_preview", False)
        
        self.load_item_preview = QCheckBox("Preview Item Thumbnail")
        self.load_seq_preview = QCheckBox("Preview Seq Thumbnail")
        self.load_item_preview.setChecked(item_preview_value)
        self.load_seq_preview.setChecked(seq_preview_value)

        h2_layout.addItem(spacer_item)
        h2_layout.addWidget(self.label_path)
        h2_layout.addItem(spacer1_item)
        h2_layout.addWidget(self.load_item_preview)
        h2_layout.addWidget(self.load_seq_preview)
        h2_layout.setAlignment(Qt.AlignTop)

        #layout.add
        #self.loadingIcon.startAnimation()
        vertical_layout.addLayout(h1_layout)
        vertical_layout.addLayout(h2_layout)
        self.setLayout(vertical_layout)

        # load preview data
        self.load_item_preview.stateChanged.connect(lambda: self.set_load_preview_mode("item_preview", self.load_item_preview))
        self.load_seq_preview.stateChanged.connect(lambda: self.set_load_preview_mode("seq_preview", self.load_seq_preview))
    ...
    def set_load_preview_mode(self, mode, checkbox):
        settings.save_preview_mode(mode, checkbox.isChecked())
    ...
    def set_path(self,path,send_update=False):
        ...
        label_path_text = path
        # if len(path.split("\\"))>1:
        #     label_path_text = path.split("\\")[1]
        self.label_path.setText(label_path_text)
...
class Project_Selector(QWidget):
    ...
    def project_context_menu_action(self,description):
        description['content'] = self.navigation.path.text()
        description['func'](description['content'])
        self.emit(SIGNAL("project_context_menu_action"),description)
    ...
    def setup_context_menu(self):
        ...
        action_list = tool_actions.project_treeview_contextmenu_actions()
        ...
        for item in action_list:
            ...
            if item:
                ...
                self.connect(action, SIGNAL('triggered()'),
                             lambda description=item:self.project_context_menu_action(description))
    ...
    def contextMenuEvent(self,event):
        # print "self.get_selected_item(): ", self.get_selected_item()
