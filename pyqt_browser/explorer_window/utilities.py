...
def packaged_executable(names):
    ...
    dirs = [r'\\botcentral\BotScripts\Software\DAM\initiator\jpeg_render_natron\Natron-2.3.13\bin',r'\\botcentral\BotScripts\Software\DAM\ImageMagick\windows']
...
def copy(path):
    settings.set_settings_data({"clipboard": path})

def paste(to_path):
    from_path = settings.get_settings_data().get("clipboard", None)
    if from_path:
        if os.path.exists(from_path):
            winshell.copy_file(from_path, str(to_path))
        else:
            print "Copied Folder or File are Not Exists."
